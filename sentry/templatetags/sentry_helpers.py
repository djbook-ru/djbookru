...
def chart_data(group, max_days=90):
    ...
    min_date = today - timedelta(hours=hours)
    ...
    #just skip zeroes
    first_seen = hours
    while not rows.get(today - timedelta(hours=first_seen)) and first_seen > 24:
        first_seen -= 1

    return {
        'points': [rows.get(today-timedelta(hours=d), 0) for d in xrange(first_seen, -1, -1)],
        'categories': [str(today-timedelta(hours=d)) for d in xrange(first_seen, -1, -1)],
    }
