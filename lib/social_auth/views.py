...
from social_auth.context_processors import backends_data
from django.contrib import messages
...
def disconnect(request, backend, association_id=None):
    ...
    if not request.user.has_usable_password():
        backens = backends_data(request.user)
        can_disconnect_accounts = len(backens['associated']) > 1
    else:
        can_disconnect_accounts = True

    if can_disconnect_accounts:
        backend.disconnect(request.user, association_id)
    else:
        messages.error(request, _(u'You can\'t disconnect last connected account until you don\'t have a password. Set password please.'))
