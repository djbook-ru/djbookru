...
class TagCloudForDeputyNode(Node):
    def __init__(self, model, context_var, **kwargs):
        self.model = model
        self.context_var = context_var
        self.kwargs = kwargs
    ...
    def render(self, context):
        model = get_model(*self.model.split('.'))
        filters = {
            'menu__parliamentarian': context['deputy'],
            'pub_date__lte': datetime.now()
        }
        if model is None:
            raise TemplateSyntaxError(_('tag_cloud_for_model tag was given an invalid model: %s') % self.model)
        context[self.context_var] = \
            Tag.objects.cloud_for_model(model, filters=filters, **self.kwargs)
        return ''
...
def do_tag_cloud_for_deputy(parser, token):
    bits = token.contents.split()
    len_bits = len(bits)
    if len_bits != 4 and len_bits not in range(6, 9):
        raise TemplateSyntaxError(_('%s tag requires either three or between five and seven arguments') % bits[0])
    ...
    return TagCloudForDeputyNode(bits[1], bits[3], **kwargs)
...
register.tag('tag_cloud_for_deputy', do_tag_cloud_for_deputy)
